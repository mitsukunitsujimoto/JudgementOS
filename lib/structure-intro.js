...
import { splitDumpToAxes } from './axes-roles.js';
...
{
  "realization": "実現したいこと＝判断の目的（1文）",
  "protection": "守りたいもの＝過程で安易に犠牲にしたくないもの（1文）",
  "constraints_recommend": ["制約タグ候補1", "制約タグ候補2"]
}
役割の非対称（最重要）:
- realization は前へ進む方向。新しい市場、売上、価値提供、事業の成立、時間の使い方など。
- protection は目的ではない。信頼・ブランド・一貫性・家族との時間など、進み方を規定する。
- 「失敗したくない」「失いたくない」「避けたい」「維持したい」だけを realization にしない。それは protection か、避けた先の目的を推定する。
- 「継続できないなら選ばない」「欺かない」などは realization にしない（境界条件。ここでは protection に要約してよいが、目的にはしない）。
- 守ることを判断目的に変換しない。「守るために何をするか」ではなく「実現するために何を守りながら進むか」。
ルール:
- 本人の言葉を要約する。ない事実は作らない。
...
export function mockStructureIntro({ categoryId, dumpText }) {
  const split = splitDumpToAxes(dumpText);
  if (split) {
    return {
      realization: split.realization,
      protection: split.protection,
      constraints_recommend: ['時間軸', '最低限守りたいこと']
    };
  }
